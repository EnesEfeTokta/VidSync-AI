...
async def generate_summary_with_ollama(transcript: str, conversation_id: str) -> StructuredSummary:
    system_prompt = (
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            logger.error(f"Ollama servisine bağlanırken ağ hatası oluştu. conversationId: {conversation_id}", exc_info=True)
            raise HTTPException(status_code=503, detail="Özetleme servisine ulaşılamıyor.")
        except httpx.HTTPStatusError as e:
            logger.error(f"Ollama API'den hata yanıtı alındı. Detay: {e.response.text}. conversationId: {conversation_id}", exc_info=True)
            raise HTTPException(status_code=500, detail="Özetleme servisinde bir API hatası oluştu.")
        except Exception as e:
            logger.error(f"Özetleme sırasında beklenmedik bir hata oluştu. conversationId: {conversation_id}", exc_info=True)
...
@app.post("/summarize", response_model=DetailedSummaryResponse, summary="Bir Sesli Konuşmayı Özetler ve Bilgileri Çıkarır")
async def summarize_conversation_endpoint(payload: MeetingPayload):
    ...
    return DetailedSummaryResponse(structured_summary=dynamic_summary, participants=payload.participants)
...
async def summarize_chat_endpoint(payload: ChatPayload):
    ...
    summary_obj = await generate_summary_with_ollama(formatted_chat, conversation_id)
    return {"summary": summary_obj.summary, "participants": payload.participants}
